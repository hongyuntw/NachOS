		...
		: ThreadedKernel(argc, argv)
{
    ...
    for (int i = 1; i < argc; i++) {
		...
		else if(strcmp(argv[i],"-prio") == 0 ){ //read prio
			execfile_prio[execfileNum] = atoi(argv[++i]);
		}
...
UserProgKernel::Initialize()
{
    ...
    //to save the page which main memory out of range
    vm_Disk = new SynchDisk("New vm_Disk");
#ifdef FILESYS
...
UserProgKernel::Run()
{
	...
	for (int n=1;n<=execfileNum;n++)
		...
		// set priority
		t[n]->setPriority(execfile_prio[n]);
