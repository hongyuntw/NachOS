    ...
    if (tlb == NULL) {		// => page table => vpn is index into table
		...
		//	    initial type
	    	...
	    	if(j<NumPhysPages){
				...
				kernel->vm_Disk->ReadSector(pageTable[vpn].virtualPage,buffer);
				...
				//LRU
				if(kernel->machine->type==0){
		    		min = kernel->machine->tab[0]->count;
		    		chosen=0;
		    		for(int m=0;m<32;m++){
		    			// printf("num%d count is;%d\n",m,tab[m]->count);
						if(min > kernel->machine->tab[m]->count){
				   			min = kernel->machine->tab[m]->count;
				    		chosen = m;
						}
		    		}
				}//FIFO
				...
				//read page to buffer2
				kernel->vm_Disk->ReadSector(pageTable[vpn].virtualPage,buffer2);

				kernel->machine->tab[chosen]->virtualPage=pageTable[vpn].virtualPage;
				kernel->machine->tab[chosen]->valid=FALSE;
				//chosen page record to buffer1
				bcopy(&mainMemory[chosen*PageSize],buffer1,PageSize);
				//buffer1 write to vm_disk	
				kernel->vm_Disk->WriteSector(pageTable[vpn].virtualPage,buffer1);
				...
				bcopy(buffer2,&mainMemory[chosen*PageSize],PageSize);
...
//	    	return PageFaultException;
		...
		// printf("TotalNum: %d \n",TotalNum);
	// printf("entry num: %d",entry->count);

    } 
    else {
    	printf("using tlb now\n");
